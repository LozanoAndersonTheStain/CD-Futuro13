...
interface DatabaseStructure {
  ...
  tournaments2?: { [key: string]: TournamentData };
}
...
export class TournamentService {
  ...
  private tournaments2Subject = new BehaviorSubject<Tournament[]>([]);

  tournaments$ = this.tournamentsSubject.asObservable();
  tournaments1$ = this.tournaments1Subject.asObservable();
  tournaments2$ = this.tournaments2Subject.asObservable();

  // Observable combinado para obtener todos los torneos
  allTournaments$ = combineLatest([
    this.tournaments$,
    this.tournaments1$,
    this.tournaments2$,
  ]).pipe(
    map(([tournaments, tournaments1, tournaments2]) => [
      ...tournaments,
      ...tournaments1,
      ...tournaments2,
    ])
  ...
  private initFirebaseListeners(): void {
    ...
    if (!environment.firebase?.databaseURL) {
      console.error(
        '❌ Firebase no está configurado correctamente. Verificar variables de entorno.'
      );
      return;
    }
  ...
  private initLocalData(): void {
    ...
    this.http
      .get<DatabaseStructure>('/assets/data/tournaments.json')
      .subscribe({
        next: (data) => {
          this.ngZone.run(() => {
            // Procesar torneos (Copa Kalley)
            if (data.tournaments) {
              const flattenedTournaments = this.flattenTournaments(
                data.tournaments,
                'copa'
              );
            ...
            if (data.tournaments1) {
              const flattenedTournaments1 = this.flattenTournaments(
                data.tournaments1,
                'liga'
              );
            ...
            // Procesar tournaments2 (Copa Amistad)
            if (data.tournaments2) {
              const flattenedTournaments2 = this.flattenTournaments(
                data.tournaments2,
                'copa'
              );
              this.tournaments2Subject.next(flattenedTournaments2);
            }
  ...
  private initTournamentsListener(): void {
    ...
    onValue(
      ...
      (error) => {
        ...
        console.error(
          '🔍 Verifica las reglas de Firebase y la configuración de red'
        );
        this.tournamentsSubject.next([]);
      }
    );
  ...
  private initTournaments1Listener(): void {
    ...
    console.log(
      '👂 Configurando listener para tournaments1 (Liga Antioquia)...'
    );
      ...
      (snapshot) => {
        this.ngZone.run(() => {
          ...
          if (tournaments1) {
            ...
            console.log(
              '🏆 Torneos procesados (Liga Antioquia):',
              flattenedTournaments1.length,
              'categorías'
            );
            this.tournaments1Subject.next(flattenedTournaments1);
          } else {
  ...
  private initTournaments2Listener(): void {
    const tournaments2Ref = ref(this.db, 'tournaments2');
    console.log('👂 Configurando listener para tournaments2 (Copa Amistad)...');

    onValue(
      tournaments2Ref,
      (snapshot) => {
        this.ngZone.run(() => {
          const tournaments2 = snapshot.val();
          console.log('📨 Datos recibidos de tournaments2:', tournaments2);
          ...
          if (tournaments2) {
            ...
            console.log('📭 No hay datos en tournaments2');
            this.tournaments2Subject.next([]);
          }
        });
      },
      (error) => {
        console.error('❌ Error al escuchar tournaments2:', error);
        this.tournaments2Subject.next([]);
      }
    );
  }

  private flattenTournaments(
    tournaments: { [key: string]: TournamentData },
    type: 'copa' | 'liga',
    cupName?: string
  ): Tournament[] {
    const result: Tournament[] = [];
    Object.entries(tournaments).forEach(([tournamentId, tournamentData]) => {
      Object.entries(tournamentData.categories).forEach(
        ([categoryId, categoryData]) => {
          result.push({
            id: categoryId,
            tournamentId,
            tournamentName: tournamentData.tournamentInfo.name,
            tournamentType: type,
            category: categoryData.category,
            imageTournament: categoryData.imageTournament,
            matches: categoryData.matches,
            title: categoryData.title,
            year: categoryData.year,
            phase: categoryData.phase,
            cupName: cupName || tournamentData.tournamentInfo.name,
          });
        }
      );
    });
  ...
  getTournamentPhases(
    ...
    return this.allTournaments$.pipe(
      map((tournaments) =>
        tournaments.filter(
          (t) => t.tournamentId === tournamentId && t.category === category
        )
      )
    );
  }
    ...
    if (this.useLocalData) {
      return this.http
        ...
        .pipe(
              ...
              Object.keys(data.tournaments).forEach((tournamentKey) => {
                ...
                if (tournament.categories && tournament.categories[id]) {
                  foundTournament = {
                    ...
                    tournamentName:
                      tournament.tournamentInfo?.name || 'Torneo Desconocido',
                    tournamentType: 'copa',
          ...
          catchError((error) => {
            console.error(
              '❌ Error obteniendo torneo desde datos locales:',
              error
            );
            return [null];
          })
      ...
      return new Observable<Tournament | null>((observer) => {
        ...
        const unsubscribe1 = onValue(
          ...
          (error) => {
            console.error(
              '❌ Error getting tournament from tournaments:',
              error
            );
            checkComplete();
          }
        ...
        const unsubscribe2 = onValue(
          ...
          (error) => {
            console.error(
              '❌ Error getting tournament from tournaments1:',
              error
            );
            checkComplete();
          }
  ...
  switchDataSource(useLocal: boolean): void {
    console.log(
      `🔄 Cambiando fuente de datos a: ${useLocal ? 'Local' : 'Firebase'}`
    );
  ...
  getTournamentInfos(): Observable<any[]> {
    return combineLatest([
      ...
      map(([copaData, ligaData, copaAmistadData]) => {
        ...
        // Procesar Copa Amistad
        if (copaAmistadData.length > 0) {
          const firstCopaAmistad = copaAmistadData[0];
          tournamentInfos.push({
            name: firstCopaAmistad.tournamentName,
            year: firstCopaAmistad.year,
            description: 'Copa Amistad de Asobdim 2025',
            startDate: '2025-08-06',
            endDate: '2025-11-30',
            status: 'active',
            tournamentId: firstCopaAmistad.tournamentId,
            type: 'copa',
            categoriesCount: copaAmistadData.length,
          });
        }
  ...
  testFirebaseConnection(): void {
    ...
    onValue(testRef, (snapshot) => {
      ...
      if (connected) {
        ...
        console.log(
          '🔧 Verificar conexión a internet y configuración de Firebase'
        );
      }
    });
  }
  ...
  diagnosticFirebaseConfig(): void {
    ...
    console.log(
      '🔑 API Key:',
      environment.firebase?.apiKey ? '✅ Configurado' : '❌ Faltante'
    );
    console.log(
      '🏠 Auth Domain:',
      environment.firebase?.authDomain ? '✅ Configurado' : '❌ Faltante'
    );
    console.log(
      '🗄️ Database URL:',
      environment.firebase?.databaseURL ? '✅ Configurado' : '❌ Faltante'
    );
    console.log(
      '📁 Project ID:',
      environment.firebase?.projectId ? '✅ Configurado' : '❌ Faltante'
    );
    console.log(
      '🏪 Storage Bucket:',
      environment.firebase?.storageBucket ? '✅ Configurado' : '❌ Faltante'
    );
